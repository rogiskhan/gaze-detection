...
/// </summary>
public abstract class MyWebCamera : MonoBehaviour
{
    /// <summary>
    ...
    public string DeviceName
    {
        ...
        set
        {
            ...
            // get device index
            int cameraIndex = -1;
            Debug.Log("Number of video capture devices: " + WebCamTexture.devices.Length);
            for (int i = 0; i < WebCamTexture.devices.Length && -1 == cameraIndex; i++)
            {
                ...
                {
                    cameraIndex = i;
                    Debug.Log("Name of device ["+i+"]: " + WebCamTexture.devices[i].name);
                }
